...
func FileTui() error {
	...
	if !client.FileExists(ans.Path) {
		return fmt.Errorf("file \"%s\" not exist", ans.Path)
	}

	app := app.GetApplication()

	err = app.CreateFileSecret(ans.Metadata, ans.Path)
	if err != nil {
		return err
	}
	...
	case tea.KeyMsg:
		...
		case "tab", "shift+tab", "enter", "up", "down":
			...
			if s == "enter" && m.focusIndex == len(m.inputs) {
				m.Metadata = m.inputs[0].Value()
				m.Path = m.inputs[1].Value()
				m.Done = true
				return m, tea.Quit
			}
...
func InitialModel() Model {
	...
	for i := range m.inputs {
		...
		case 0:
			...
			t.Focus()
		case 1:
			t.Placeholder = "Absolute Path to the file"
